...
#define	SOUNDOPT
#ifdef	SOUNDOPT
#include "nifopt-wav.cpp"

bool process_wav(const char *inname, const char *ouname, const char *rep) {
  st_signalinfo_t filein; ft_t descin;
  st_signalinfo_t fileou; ft_t descou;

  memset(&filein, 0, sizeof(filein)); descin = NULL;
  memset(&fileou, 0, sizeof(fileou)); descou = NULL;
  ...
  if ((descin = st_open_read(inname, &filein, "wav"))) {
    ...
    st_report("Input file %s: using sample rate %lu\n\tsize %s, encoding %s, %d %s",
      descin.filename, filein.rate,
      st_sizes_str[(unsigned char)filein.size],
      st_encodings_str[(unsigned char)filein.encoding],
      filein.channels,
      (filein.channels > 1) ? "channels" : "channel");

    if (descin.comment)
      st_report("Input file %s: comment \"%s\"\n", descin.filename, descin.comment);
#endif
    ...
    if (fileou.rate     ==  0) fileou.rate     = descin->info.rate;
    if (fileou.size     == -1) fileou.size     = descin->info.size;
    if (fileou.encoding == -1) fileou.encoding = descin->info.encoding;
    if (fileou.channels == -1) fileou.channels = descin->info.channels;

    /* lossless */
    fileou.size     = min(filein.size, ST_SIZE_WORD);
    fileou.encoding = ST_ENCODING_SIGN2;

#if 0
    /* a-Law 1:2 */
    fileou.size     = ST_SIZE_WORD;
    fileou.encoding = ST_ENCODING_ALAW;

    /* ADPCM 1:4 LQ */
    fileou.size     = ST_SIZE_BYTE;
    fileou.encoding = ST_ENCODING_IMA_ADPCM;
#endif

    if (compresssounds) {
      /* ADPCM 1:4 HQ */
      fileou.size     = ST_SIZE_BYTE;
      fileou.encoding = ST_ENCODING_ADPCM;
    }
     ...
     */
    double factor = (double)fileou.rate / (double)filein.rate;
    ...
    if ((descou = st_open_write_instr(ouname, &fileou, "wav", descin->comment, &descin->instr, loops))) {
      wav_t wavin = (wav_t)descin->priv;
      wav_t wavou = (wav_t)descou->priv;
       ...
       * (or ST_EOF).
       */
      st_ssize_t ilen, olen;
      st_sample_t *buffer = new st_sample_t[ST_BUFSIZ];
      while ((ilen = st_read(descin, buffer, (st_ssize_t)ST_BUFSIZ)) > 0) {
	olen = st_write(descou, buffer, (st_ssize_t)ilen);
	/* we can interrupt this even before triggering EOF */
	if (ilen < ST_BUFSIZ)
	  break;
      }
      ...
      st_flush_close(descin);
      st_flush_close(descou);
	...
	for (size_t n = 0; n < notes.size(); n++)
      ...
      /* uh, MicroSoft, no signed "ssize_t"? */
      size_t is, os;
      if ((is = (size_t)st_tell(descin)))
	processedinbytes += is, convertedinbytes += is, deltaiobytes  = is;
      if ((os = (size_t)st_tell(descou)))
	processedoubytes += os, convertedoubytes += os, deltaiobytes -= os;

      success = true;
    }
  }

  if (descou) st_close(descou);
  if (descin) st_close(descin);
